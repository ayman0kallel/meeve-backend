...
export const createSportTable = async () => {
  try {
    ...
    const [results] = await db.execute(preFillDataQuery);

    console.log('Sport table created and pre-filled with data:', results);
  } catch (error) {
    console.error('Error creating sport table or pre-filling data:', error);
  }
};